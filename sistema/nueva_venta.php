	...
	<section id="container">
		...
		<div style="flex-wrap: wrap;">
			...
			<div class="data_producto">
				<br>
					<input type="text" name="busquedaProd" id="busquedaProd" placeholder="Buscar por código o descripción">
				<br>
			...
			<table class="table_venta">
				<tr>
					<td><input type="hidden" name="idcliente" id="idcliente" value="" required>
						<input type="text" name="nom_cliente" id="nom_cliente" placeholder="Buscar cliente" required>
						<input type="text" name="nit_cliente" id="nit_cliente" disabled placeholder="DNI">
				    ...
				    $('#nom_cliente').autocomplete({
				        ...
				        select: function(event, ui) {
				            $.ajax({
				                url: 'ajax.php',
				                type: 'POST',
				                data: {action: 'searchCliente', cliente: ui.item.value},
				                success: function(response) {
				                    if(response != 'error') {
				                        var info = JSON.parse(response);
				                        $('#idcliente').val(info.idcliente);
				                        $('#nit_cliente').val(info.ruc);
